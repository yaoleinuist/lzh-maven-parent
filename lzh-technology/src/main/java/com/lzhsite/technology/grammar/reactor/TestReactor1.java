...
import com.lzhsite.entity.Student;
...
public class TestReactor1 {
	...
	public void testSimpleFluxMono() {
		...
		Mono.error(new Exception("some error")).subscribe(System.out::println, System.err::println,
				() -> System.out.println("Completed!"), s -> System.out.println("Subscribed!"));
	}
	...
	public void testSimpleOperators() throws InterruptedException {
		// map + filter
		StepVerifier.create(Flux.range(1, 6).filter(i -> i % 2 == 1).map(i -> i * i)).expectNext(1, 9, 125)
				.verifyComplete();

		  // flatMap
		  StepVerifier.create(Flux.just("flux", "mono")
		 .flatMap(s ->
		 Flux.fromArray(s.split("\\s*")).delayElements(Duration.ofMillis(100)))
		 ...
		 .subscribe(t -> System.out.println(t.getT1()), null,
	...
	public void testAppendBoomError() {
		...
		StepVerifier.create(appendBoomError(source)).expectNext("foo").expectNext("bar").expectErrorMessage("boom")
				.verify();
	}
	...
	public void testSyncToAsync() throws InterruptedException {
		CountDownLatch countDownLatch = new CountDownLatch(1);
		Mono.fromCallable(() -> getStringSync()).subscribeOn(Schedulers.elastic()).subscribe(System.out::println, null,
				countDownLatch::countDown);
		countDownLatch.await(10, TimeUnit.SECONDS);
	}
	...
	public void testErrorHandling() throws InterruptedException {
		// 0. 在订阅的时候进行错误处理
		Flux.range(1, 6).map(i -> 10 / (i - 3)).map(i -> i * i).subscribe(System.out::println, System.err::println);

		// 1. 遇到错误信号给出缺省值
		Flux.range(1, 6).map(i -> 10 / (i - 3)).onErrorReturn(0).map(i -> i * i).subscribe(System.out::println,
				System.err::println);

		// 2. 捕获并执行一个异常处理方法或动态计算值顶替
		Flux.range(1, 6).map(i -> 10 / (i - 3)).onErrorResume(e -> Mono.just(new Random().nextInt(6))).map(i -> i * i)
				.subscribe(System.out::println, System.err::println);

		// 3. 捕获，并再包装为某一个业务相关的异常，然后再抛出业务异常
		Flux.range(1, 6).map(i -> 10 / (i - 3)).onErrorMap(e -> new RuntimeException("被零除异常")).map(i -> i * i)
				.subscribe(System.out::println, System.err::println);

		// 4. 只读地记录日志
		Flux.range(1, 6).map(i -> 10 / (i - 3)).doOnError(e -> System.out.println("记录日志：" + e)).map(i -> i * i)
				.subscribe(System.out::println, System.err::println);
		...
		// 6. 重试
		Thread.sleep(1000);
		Flux.range(1, 6).map(i -> 10 / (3 - i)).retry(1).subscribe(System.out::println, System.err::println);
		Thread.sleep(100);
	}
	...
	public void testBackpressure() {
		Flux.range(1, 6).log().subscribe(new BaseSubscriber<Integer>() {
