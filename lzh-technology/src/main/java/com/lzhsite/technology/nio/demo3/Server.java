...
public class Server implements Runnable {
		...
		while (true) {
			try {
				...
				if (num > 0) {
					...
					addRegister(); // 在Selector中注册新的写通道,从读线程中转回来主线程。
				}
			} catch (Exception e) {
	...
	private void addRegister() {
		synchronized (wpool) {
			while (!wpool.isEmpty()) {
				System.out.println("wpool is not empty");
	...
	/**
	 * 提交新的客户端写请求于主服务线程的回应池中，加上 synchronized 保证读线程 通道和写线程通道是同一个KEY。防止窜包。
	 */
	public static void processWriteRequest(SelectionKey key) {
		...
		selector.wakeup(); // 解除selector的阻塞状态，以便注册新的通道 。主要是因为在不同线程中。从读线程中转回来主线程。
		System.out.println("selector is wakeup");
	}
}
